"""
"""
from cspkg.plugins.syntax.tools import get_tokens_matrix
...
class Spider(Plugin):
    ...
    def  __init__(self, xstr, max=10):
        ...
        if default_style:
            self.xstr.configure(foreground=default_style)
    ...
    def conf_tokstyle(self, tokname, tokstyle):
        ...
        if not foreground is None:
            self.xstr.tag_config(tokname, foreground='#%s' % foreground)
        if not background is None:
            self.xstr.tag_config(tokname, background='#%s' % background)
        if not underline is None:
            self.xstr.tag_config(tokname, underline=underline)
        self.xstr.tag_lower(tokname, 'sel')
    ...
    def update_all(self, event):
        ...
        self.lexer = findlexer(self.xstr.filename, 
        self.xstr.get('1.0', '1.0 +258c') if self.guesslex else None, 
    ...
    def update(self, event):
        ...
        for ttype, tokstyle in self.style.styles.items():
            self.xstr.tag_remove(str(ttype), index2, index6)
        data = self.xstr.get(index2, index6)
